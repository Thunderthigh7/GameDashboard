...
const MAX_RELEASE_COMPARISON_WINDOW_MS = 7 * 24 * 60 * 60 * 1000;
...
export function buildReleaseComparison(options = {}) {
  ...
  const rawFindingItems = ready
    ? buildDeterministicFindings({ coreMetrics, funnels, eventsByName })
    : [];
...
function buildTrafficMatchedComparison(context) {
  ...
  const comparisonWindow = buildReleaseAgeComparisonWindow(context.beforeCohort, context.afterCohort);
  const beforeSessions = sessionCatalog.filter((session) => (
    session.placeVersion === cleanInteger(context.beforeCohort?.placeVersion)
    && sessionFallsInsideReleaseWindow(session, comparisonWindow.before)
  ));
  const afterSessions = sessionCatalog.filter((session) => (
    session.placeVersion === cleanInteger(context.afterCohort?.placeVersion)
    && sessionFallsInsideReleaseWindow(session, comparisonWindow.after)
  ));
  ...
  if (matchedSessionCount < 1) {
    return {
      ...
      dimensions: ["release_age_window", "player_lifecycle", "platform"],
      comparisonWindow,
  ...
  const rawFindingItems = status === "ready"
    ? buildDeterministicFindings({ coreMetrics, funnels, eventsByName: events })
    : [];
  const findingItems = rawFindingItems.map((finding) => ({
    ...finding,
    confidence: platformCoverageSufficient ? finding.confidence : "directional",
    method: `Compared inside the same post-release window, then exact-matched by first-seen/returning status and observed platform. ${finding.method}${platformCoverageSufficient ? "" : " Platform coverage is below the recommended threshold, so confidence is directional."}`,
  }));
  ...
  return {
    ...
    dimensions: ["release_age_window", "player_lifecycle", "platform"],
    comparisonWindow,
    releaseWindowDefinition: "Both versions use the same amount of time after their first production observation, capped at seven days.",
    ...
    findings: {
      ...
      note: platformCoverageSufficient
        ? "Findings use the same post-release window and equal-sized exact matches inside every shared first-seen/returning and platform stratum."
        : `Findings use the same post-release window and lifecycle matching, but observed platform coverage is below ${MIN_PLATFORM_COVERAGE_PERCENT}% on at least one side and is labeled directional.`,
    },
  };
}

function buildReleaseAgeComparisonWindow(beforeCohort, afterCohort) {
  const beforeStart = cleanInteger(beforeCohort?.firstSeenAt);
  const beforeLast = cleanInteger(beforeCohort?.lastSeenAt);
  const afterStart = cleanInteger(afterCohort?.firstSeenAt);
  const afterLast = cleanInteger(afterCohort?.lastSeenAt);
  const beforeDurationMs = Math.max(0, beforeLast - beforeStart);
  const afterDurationMs = Math.max(0, afterLast - afterStart);
  const enabled = beforeStart > 0 && afterStart > 0 && beforeDurationMs > 0 && afterDurationMs > 0;
  const durationMs = enabled
    ? Math.min(MAX_RELEASE_COMPARISON_WINDOW_MS, beforeDurationMs, afterDurationMs)
    : null;
  return {
    enabled,
    durationMs,
    maximumDurationMs: MAX_RELEASE_COMPARISON_WINDOW_MS,
    before: enabled ? { startAt: beforeStart, endAt: beforeStart + durationMs } : null,
    after: enabled ? { startAt: afterStart, endAt: afterStart + durationMs } : null,
  };
}

function sessionFallsInsideReleaseWindow(session, window) {
  if (!window) return true;
  const timestamp = cleanInteger(session?.firstObservedAt);
