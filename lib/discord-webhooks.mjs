...
export async function sendDiscordWebhookAlert({
  ...
  const title = cleanDiscordText(alert?.title, 256) || "RoSignal alert";
  const description = cleanDiscordText(alert?.description, 2000);
  ...
  try {
    response = await fetchImpl(targetUrl, {
      ...
      headers: {
        ...
        "User-Agent": "RoSignal-Discord-Integration/1.0",
      },
