...
const AREAS = {
  hub: { name: "Analytics Hub", x: 0, y: 4, z: 0, gameMode: "Hub", map: "RoAnalytics Demo World", mapId: "demo-hub" },
  ...
  swordArena: { name: "Sword Duel Arena", x: 66, y: 4, z: 74, gameMode: "Sword Arena", map: "Temple Ring", mapId: "sword-temple" },
  simulator: { name: "Pet Simulator Plaza", x: 132, y: 4, z: 76, gameMode: "Pet Simulator", map: "Hatchery", mapId: "sim-hatchery" },
  shop: { name: "Robux Shop", x: 0, y: 4, z: -72, gameMode: "Commerce", map: "Starter Plaza", mapId: "demo-shop" },
};
...
function createCustomEvents(random, sessions) {
  ...
  const addContext = (area) => ({
    ...
    mapName: area?.map || "RoAnalytics Demo World",
  });
  ...
  for (const session of sessions) {
    ...
    if (genre === "Obby") {
      add(session, "obby_run_started", 2, AREAS.obbyStart, {
        course: "Skyline Obby",
        difficulty: "Hard",
        checkpointCount: 7,
        startBoostEnabled: random() < 0.12,
      });
      add(session, "obby_checkpoint_reached", 4, OBBY_OBSTACLES["Warmup Steps"], {
        checkpoint: "Warmup Steps",
        checkpointIndex: 1,
      });
      ...
      if (reachedMovingPlatforms) add(session, "obby_checkpoint_reached", 7, AREAS.obbyEasy, {
        checkpoint: "Moving Platforms",
        checkpointIndex: 2,
      });
      if (reachedSpikeHall) add(session, "obby_checkpoint_reached", 10, OBBY_OBSTACLES["Spike Hall"], {
        checkpoint: "Spike Hall",
        checkpointIndex: 3,
      });
      if (reachedLaserLadder) add(session, "obby_checkpoint_reached", 13, AREAS.obbyHard, {
        checkpoint: "Laser Ladder",
        checkpointIndex: 4,
      });
      ...
      for (let failure = 0; failure < failureCount; failure += 1) {
        ...
        add(session, "obby_failed", 6 + failure * 3, OBBY_OBSTACLES[obstacle], {
          ...
          recoveriesToAttempt: 1 + (failure % 3),
          secondsSinceSpawn: 34 + (failure * 4),
        });
      }

      if (completed) {
        add(session, "obby_checkpoint_reached", 16, OBBY_OBSTACLES["Final Jump"], {
          checkpoint: "Final Jump",
          checkpointIndex: 5,
          momentumBoost: 12 + (index % 5),
        });
        add(session, "obby_completed", 18, AREAS.obbyFinish, {
          course: "Skyline Obby",
          attempts: failureCount + 1,
          completionTimeSeconds: 185 + (index % 170),
          bestCompletionTimeSeconds: 160 + (index % 140),
          completionLane: (index % 3) + 1,
        });
      }
    } else if (genre === "FPS") {
      const selectedWeapon = pickWeightedValue(fpsLoadoutIndex, FPS_WEAPON_USAGE);
      fpsLoadoutIndex += 1;
      add(session, "fps_match_joined", 2, AREAS.armory, {
        map: "Cargo Yard",
        mode: "Team Deathmatch",
        matchType: "Round-Based",
      });
      ...
      for (let death = 0; death < deathCount; death += 1) {
        ...
        add(session, "combat_death", 7 + death * 4, AREAS.fpsArena, {
          ...
          roundTimeSeconds: 42 + (death * 7) + (index % 18),
      ...
      if (random() < (isCurrentRelease ? 0.68 : 0.77)) {
        add(session, "fps_match_completed", 17, AREAS.fpsArena, {
          ...
          kills: 2 + (index % 17),
          deaths: 1 + (index % 4),
          friendlyFireDamage: random() < 0.08,
        });
      }
      ...
      for (let defeat = 0; defeat < defeatCount; defeat += 1) {
        ...
        add(session, "sword_duel_defeat", 7 + defeat * 4, AREAS.swordArena, {
          ...
          selectedSword,
          arena: "Temple Ring",
          comboCount: 2 + (defeat % 5),
          dodgeWindowMs: 180 + (defeat * 35),
        });
      }
      ...
      if (random() < 0.74) {
        add(session, "sword_duel_completed", 15, AREAS.swordArena, {
          ...
          duelDurationSeconds: 88 + (index % 80),
        });
      }
    } else {
      add(session, "simulator_zone_entered", 2, AREAS.simulator, {
        zone: "Pet Hatchery",
        hatcheryMode: "Campaign",
      });
      ...
      for (let hatch = 0; hatch < hatchCount; hatch += 1) {
        ...
        add(session, "egg_hatched", 4 + hatch * 2, AREAS.simulator, {
          egg,
          rarity,
          petType: rarity === "Legendary" ? "Sky Lizard" : "Pocket Drak",
          hatchSkill: hatch % 2 === 0 ? "Timed" : "Manual",
          hatchTimeSeconds: 7 + (hatch * 2),
        });
      }
    ...
    if (random() < 0.46) {
      ...
      if (purchased) {
        add(session, "item_purchased", 10, AREAS.shop, {
          product,
          priceRobux,
          currency: "Robux",
          revenueTier: priceRobux >= 120 ? "Premium" : "Starter",
        });
      } else {
        ...
        shopCloseIndex += 1;
        add(session, "purchase_prompt_closed", 10, AREAS.shop, {
          reason: closeReason,
          product,
          priceRobux,
        });
...
function createDemoFunnels(referenceTime) {
  return [
    ...
    },
    {
      ...
      steps: ["fps_match_joined", "weapon_selected", "combat_death"],
      conversionWindowMinutes: 30,
      createdAt: referenceTime,
      updatedAt: referenceTime,
    },
    {
      id: "demo-funnel-sword-duel",
      name: "Sword duel outcomes",
      steps: ["sword_duel_started", "sword_selected", "sword_duel_defeat"],
      conversionWindowMinutes: 30,
      createdAt: referenceTime,
      updatedAt: referenceTime,
    },
