...
export function createDemoUniverseFixture(options = {}) {
  ...
  const deathSamples = createSignalSamples(random, sessions, "death", 420);
...
function createDemoSessions(random, players, referenceTime) {
  return Array.from({ length: DEMO_SESSION_COUNT }, (_, index) => {
    ...
    return {
      ...
      startedAt,
      ...getDemoVersionForTimestamp(startedAt, index, referenceTime),
...
function createCustomEvents(random, sessions) {
  ...
  for (const session of sessions) {
    ...
    const tutorialCompleted = random() < (isCurrentRelease ? 0.72 : 0.88);
    ...
    if (tutorialCompleted && random() < (isCurrentRelease ? 0.62 : 0.76)) add(session, "first_upgrade", 10, AREAS.shop, { item: ["Speed", "Health", "Power"][index % 3], currencySpent: 100 + (index % 5) * 25 });
    ...
    const defeatedBoss = random() < (isCurrentRelease ? 0.25 : 0.42);
    ...
    if (random() < (isCurrentRelease ? 0.21 : 0.11)) add(session, "item_purchased", 12, AREAS.shop, { product: ["Starter Pack", "Double Coins", "Revive"][index % 3], priceRobux: [99, 149, 49][index % 3] });
