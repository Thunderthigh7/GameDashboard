...
export function calculateFunnelAnalytics(definition, sessions, options = {}) {
  ...
  const conversionWindowMs = getFunnelConversionWindowMs(definition);
  const sessionList = Array.isArray(sessions) ? sessions : [];

  for (const session of sessionList) {
    const matchedEvents = matchFunnelSessionEvents(eventNames, session?.events, conversionWindowMs);
    const firstTimestamp = getEventTimestamp(matchedEvents[0]);
    if (!firstTimestamp || !isFunnelEntryTimestampInRange(firstTimestamp, options)) continue;

    let previousTimestamp = 0;
    for (let stepIndex = 0; stepIndex < matchedEvents.length; stepIndex += 1) {
      const matchedTimestamp = getEventTimestamp(matchedEvents[stepIndex]);
    ...
    if (matchedEvents.length === steps.length && steps.length) {
      completionDurations.push(previousTimestamp - firstTimestamp);
    }
  }
  ...
  const requestedTrackedSessionCount = Number(options?.totalTrackedSessions);
  const totalTrackedSessions = Number.isSafeInteger(requestedTrackedSessionCount) && requestedTrackedSessionCount >= 0
    ? requestedTrackedSessionCount
    : sessionList.length;
...
export function calculateFunnelTimelineAnalytics(definition, sessions, buckets) {
  ...
  return cleanBuckets.map((bucket) => ({
    ...
    steps: eventNames.map((eventName, index) => {
      const sessions = bucket.stepSessions[index];
      const previousSessions = index > 0 ? bucket.stepSessions[index - 1] : bucket.entrySessions;
      return {
        index: index + 1,
        eventName,
        sessions,
        percentage: bucket.entrySessions ? roundPercentage(sessions / bucket.entrySessions) : null,
        conversionFromPrevious: previousSessions ? roundPercentage(sessions / previousSessions) : null,
      };
    }),
  }));
}

export function calculateFunnelMapSamples(
  definition,
  sessions,
  requestedStepIndex,
  requestedMode = "reached",
  options = {},
) {
  const eventNames = Array.isArray(definition?.steps) ? definition.steps.map(normalizeEventName).filter(Boolean) : [];
  ...
  for (const session of Array.isArray(sessions) ? sessions : []) {
    const matchedEvents = matchFunnelSessionEvents(eventNames, session?.events, conversionWindowMs);
    const entryTimestamp = getEventTimestamp(matchedEvents[0]);
    if (!entryTimestamp || !isFunnelEntryTimestampInRange(entryTimestamp, options)) continue;
...
export function getFunnelConversionWindowMs(definition) {
  return Math.max(cleanPositiveInteger(definition?.conversionWindowMinutes), 1) * 60 * 1000;
}
...
function isFunnelEntryTimestampInRange(timestamp, options = {}) {
  const entryFromMs = cleanTimestampMs(options?.entryFromMs);
  const entryToMs = cleanTimestampMs(options?.entryToMs);
  if (entryFromMs > 0 && timestamp < entryFromMs) return false;
  if (entryToMs > 0 && timestamp > entryToMs) return false;
  return true;
}
