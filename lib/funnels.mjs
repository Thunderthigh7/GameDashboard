...
export function calculateFunnelStepChanges(definition, sessions, ranges, options = {}) {
  ...
  const improvedSteps = steps.filter((step) => step.signal === "improved");
  const declinedSteps = steps.filter((step) => step.signal === "declined");
  ...
  return {
    ...
    steps,
    largestImprovementStepIndex: improvedSteps
      .sort((left, right) => right.changePercentagePoints - left.changePercentagePoints)[0]?.index || null,
    largestDeclineStepIndex: declinedSteps
      .sort((left, right) => left.changePercentagePoints - right.changePercentagePoints)[0]?.index || null,
  };
}
