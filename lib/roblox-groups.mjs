...
export async function listRobloxUserGroups(accessToken, robloxUserId) {
  ...
  const groupIds = [...new Set(memberships.map(membershipGroupId).filter(Boolean))];
  if (!groupIds.length) return [];

  const response = await fetch(`https://groups.roblox.com/v2/groups?groupIds=${encodeURIComponent(groupIds.join(","))}`, {
    signal: AbortSignal.timeout(ROBLOX_GROUP_REQUEST_TIMEOUT_MS),
  });
  const groupPayload = await readJsonResponse(response);
  if (!response.ok) throw createRobloxGroupError(response.status, groupPayload, "Could not load Roblox groups.");
  const groupsById = new Map(getPayloadArray(groupPayload, ["data", "groups"]).map((group) => [
