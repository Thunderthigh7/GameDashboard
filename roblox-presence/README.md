...
You can define the same shape before writing code from **Events → New event**. Valid paths received from this logger are added to the event automatically. You can hide unwanted properties from dashboard breakdowns without discarding them from the raw event record. The generated Roblox Luau is ready to copy into a server script.
